<template>
  ...
  <div class="sysSensitiveWord">
    ...
    <section class="right-content">
      <Search ref="Search" :notSearch="notSearch" :searchFormConfig="searchFormConfig" @search="_mxDoSearch"
        @create="create" @exportData="exportData">
        <template slot="Other">
      ...
      <el-table :data="listData" highlight-current-row style="width: 100%;" v-loading="loading">
        ...
        <el-table-column label="操作" width="150">
          <template slot-scope="scope">
            ...
            <el-button @click="_mxDeleteItem(scope.row.wordId)" type="text" size="small">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <el-dialog :title="createOrUpdate" :visible.sync="isAddGroup" :close-on-click-modal="false"
        :close-on-press-escape="false" :show-close="false" top="45px" width="30%">
        <el-form ref="addGroupObj" :model="addGroupObj" label-width="100px">
          <el-form-item label="敏感词组" prop="groupName"
            :rules="[{ required: true, message: '敏感词组不能为空', trigger: 'blur'}]">
            <el-input v-model="addGroupObj.groupName" clearable maxlength="15" show-word-limit placeholder="请输入敏感词组" />
          </el-form-item>
        </el-form>
        <div slot="footer">
  ...
  methods: {
    ...
    handleConfirm () {
      this.$refs.addGroupObj.validate(valid => {
        if (valid) {
          const postUrl = this.createOrUpdate === "添加敏感词组" ? this.$http.sysSensitiveWordGroup.addSensitiveWordGroup : this.$http.sysSensitiveWordGroup.updateSensitiveWordGroup;
          const data = this.createOrUpdate === "添加敏感词组" ? this.addGroupObj : { groupId: this.editId, ...this.addGroupObj }
    ...
    handleDeleteGroup (groupId) {
      this.$confirm('删除后组内的敏感词同步删除，请谨慎操作', '您确定要删除敏感词组吗？', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.$http.sysSensitiveWordGroup.deleteSensitiveWordGroup({ data: { groupId } }).then(res => {
          if (res.code === 200) {
            this.getGroupList();
            this.$message.success(res.data || res.msg);
          } else {
    ...
    handleSelectGroup (groupId) {
      this.$http.sysSensitiveWord.listSensitiveWordByPage({ data: { sensitiveWord: { groupId } } }).then(res => {
        if (res.code === 200) {
    ...
    create () {
      ...
      this.$router.push({ name: 'sysSensitiveWordType', query: { type: 'create' } });
    },
    edit (row, ID) {
      this.$router.push({ name: 'sysSensitiveWordType', query: { type: 'update', row: JSON.stringify(row), ID } });
    },
    _mxDeleteItem (wordId) {
      this.$confirm('删除后将不可找回，请谨慎操作', '您确定要删除敏感词吗？', {
        ...
        this.$http.sysSensitiveWord.deleteSensitiveWord({ data: { wordId } }).then(res => {
          if (res.code === 200) {
            this._mxGetList();
            this.$message.success(res.data || res.msg);
          } else {
