<template>
  <div>
    ...
    <el-table :data="listData" max-height="500" highlight-current-row style="width: 100%" v-loading="loading">
      <el-table-column prop="serverType" label="产品类型" show-overflow-tooltip>
        <template slot-scope="{row}">{{renderServerType(row.serverType)}}</template>
      </el-table-column>
      <el-table-column prop="operatorName" label="操作人名称" show-overflow-tooltip />
      <el-table-column prop="createTime" label="操作时间" min-width="150" show-overflow-tooltip>
        <template slot-scope="{row}">{{ row.createTime | timeFormat }}</template>
      </el-table-column>
      <el-table-column prop="requIp" label="请求IP" show-overflow-tooltip />
      <el-table-column prop="module" label="操作模块" show-overflow-tooltip />
