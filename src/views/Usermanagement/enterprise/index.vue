    ...
    >
      ...
      <el-table-column fixed="right" label="操作" width="200">
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.reductModel == 1" @click="setType(scope.row,'init')" type="text" size="small">初始
          </el-button>
          <el-button v-if="scope.row.reductModel == 2" @click="setType(scope.row,'disable')" style="color: #ec5858"
                     type="text" size="small">禁用
          </el-button>
          <el-button v-if="scope.row.reductModel == 3" @click="setType(scope.row,'enabled')" type="text" size="small">
            启用
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="formTit" :visible.sync="customerAddInfo" :close-on-click-modal="false" style="margin: 0 auto">
      <el-form ref="addForm" label-width="120px" :model="addInfo" :rules="updateFormRules" class="demo-ruleForm">
        ...
        <el-form-item label="父企业ID" prop="root">
          ...
          <span><el-button v-if="addInfo.root" @click="selectCompany">修改</el-button></span>
        </el-form-item>
      </el-form>
